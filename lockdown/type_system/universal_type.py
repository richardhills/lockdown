...
class Universal(Composite):
    def __init__(self, is_sparse, default_factory=None, bind=None, debug_reason=None):
        self.wrapped = {}
...
class PythonObject(Universal):
    def __init__(self, initial_data, **kwargs):
        super(PythonObject, self).__init__(True, **kwargs)
...
class PythonList(Universal, MutableSequence):
    def __init__(self, initial_data, **kwargs):
        super(PythonList, self).__init__(False, **kwargs)
...
class GetterMicroOpType(MicroOpType):
    ...
    def __init__(self, key, key_type, value_type):
        self.key = key
    ...
    def clone(self):
        return GetterMicroOpType(self.key, self.key_type, self.value_type)
...
class SetterMicroOpType(MicroOpType):
    ...
    def clone(self):
        return SetterMicroOpType(self.key, self.key_type, self.value_type, self.type_error)
...
class GetterWildcardMicroOpType(MicroOpType):
    ...
    def is_derivable_from(self, other_type):
        other_micro_op_type = other_type.get_micro_op_type(("get-wildcard", self.key))
        return (
            other_micro_op_type
            and other_type.key_type.is_copyable_from(self.key_type)
            and self.value_type.is_copyable_from(other_micro_op_type.value_type)
...
class InserterWildcardMicroOpType(MicroOpType):
    ...
    def merge(self, other_micro_op_type):
        return InsertWildcardMicroOpType(
    ...
    def clone(self):
        return InsertWildcardMicroOpType(self.value_type, self.key_type, self.key_error)
...
def UniversalObjectType(properties, wildcard_type=None, has_default_factory=False, name=None):
    ...
    for name, type in properties.items():
        ...
        micro_ops[("get", name)] = GetterMicroOpType(name, StringType(), type)
        if not const:
            micro_ops[("set", name)] = SetterMicroOpType(name, StringType(), type)
...
DEFAULT_READONLY_UNIVERSAL_TYPE = CompositeType({
    ("get-wildcard",): GetterWildcardMicroOpType(OneOfType([ StringType(), IntegerType() ]), AnyType(), True),
}, "DefaultReadonlyUniversalType")
