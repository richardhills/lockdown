...
def get_default_global_context():
    ...
    types = PythonObject({
        "static": PythonObject({
            ...
            "Iterable": prepare(parse("""
                function(any keyType, any valueType) => any {
                    return {
                        type: "Universal",
                        micro_ops: [{
                            type: "iter",
    ...
    return PythonObject({
        "static": PythonObject(
            spread_dict(types._get("static")._wrapped, {
                "range": prepare(
                    function_lit(
                ...
                "find": prepare(
                    function_lit(
                        list_type([
                            function_type(no_value_type(), object_template_op({
                            ...
                            function_type(list_type([ int_type() ], None), object_template_op({
                                "value": list_template_op([ object_template_op({
                                    "out": bool_type()
                                })])
                            }))
                ...
                "keys": prepare(parse("""
                    function(Iterable<any, any> list) {
                        return list *|> { continue arguement[1]; };
                    }
                """),
    #                 function_lit(
    #                     list_type([
